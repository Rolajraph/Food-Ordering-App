  ...
  return (
    <nav className="navbar">
      ...
      <div className={`navbar__links ${isMobileMenuOpen ? 'navbar__links--open' : ''}`}>
        <Link to="/menu" className="navbar__link" onClick={() => setIsMobileMenuOpen(false)}>Menu</Link>
        <Link to="/cart" className="navbar__link navbar__cart" onClick={() => setIsMobileMenuOpen(false)}>
        ...
        {isAuthenticated ? (
          <div className="navbar__profile" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div className="navbar__dropdown">
                <Link to="/orders" className="navbar__dropdown-item" onClick={() => { setIsDropdownOpen(false); setIsMobileMenuOpen(false); }}>
                  My Orders
                </Link>
                <Link to="/profile" className="navbar__dropdown-item" onClick={() => { setIsDropdownOpen(false); setIsMobileMenuOpen(false); }}>
                  Profile
                </Link>
          ...
          <>
            <Link to="/login" className="navbar__link" onClick={() => setIsMobileMenuOpen(false)}>Login</Link>
            <Link to="/register" className="navbar__link" onClick={() => setIsMobileMenuOpen(false)}>Register</Link>
          </>
        )}
      </div>
    </nav>
